...
| 184 | Polymarket 함의관계 위반 모듈 라이브 전환 | SDD완료·라이브미시작 상태였던 `research/polymarket_market_implication/` 재개. 계기: swisstony 고수익 트레이더 조사(HFT 마켓메이킹 판정) 후 유저의 "직접 봇 만들고 싶다" 요청 → 신규설계 대신 기존모듈 재개로 방향. tmux `polymarket-implication-collect`(일1회 페어발굴)+`polymarket-implication-watch`(시간당 위반감시) 가동, ensure_collectors.sh/lab_api.py 등록. 첫 사이클: 마켓192개 태깅, 후보쌍1개. QA(30~50건)+포워드N≥20 pnl 게이트 전까진 paper. 상세: `seokminal-multi-venue/docs/progress.md` 2026-08-05, [[project_polymarket_market_implication_module]] | 05d1c39(BE) |
