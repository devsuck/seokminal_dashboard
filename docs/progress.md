...
### 스페이싱 8px 그리드 통일 (같은 날, 유저 요청 "스페이싱도 8px 그리드로 통일해줘")
- 전 레포(app+components) `p/m/gap/space-[xy]` 계열 arbitrary bracket 값(`[Npx]`) 스캔 → 깨진 값 **1건**뿐(`ml-[13px]`, `research-os/governance/page.tsx:191` 증거 사슬 커넥터 라인). 나머지는 전부 Tailwind 표준 4px 스케일(0.5=2px 단위) 사용 중이라 이미 일관.
- 표준 스케일 내 8px 배수 아닌 값(`py-0.5`, `gap-1.5`, `p-3` 등)은 수백 건 존재 — 이걸 전부 8px 배수로 강제하면 뱃지/칩 등 촘촘한 HUD 요소 padding이 다 벌어지는 큰 시각적 변경이라 유저에게 강도 확인 요청 → "4px 서브그리드 유지" 선택(현재 스케일 그대로, arbitrary 깨진 값만 수정).
- `ml-[13px]` → `ml-3.5`(14px, 표준 스케일 스텝)로 수정. throwaway 스택으로 해당 커넥터 라인 확대 스크린샷 확인 — 점(dot) 중심 정렬 문제없음.
- `tsc --noEmit` 클린, `npm test -- --run` 33/33 통과.

### 다음 할 일
- 없음(스페이싱 확인 작업 완료 — 4px 서브그리드 유지로 결론, 8px 전면 강제는 보류).
...
### 데스크탑(1440x900) 재검증
"데스크탑도 하이드레이션 에러 없는지 확인해줘" 요청으로 동일 27개 라우트 데스크탑
뷰포트 재스윕 — **27/27 클린**(모바일과 동일 결과, 뷰포트 무관 확인).

### 다음 할 일
- 없음. 유저 요청("전부 훑어봐" + 데스크탑 재확인) 완료 — 모바일/데스크탑 총 54회
  라우트 로드 전수 확인, 추가 조치 불필요
...
### 다음 할 일
- ~~`npm audit fix --force`(next 업그레이드) 여부~~ — 유저 결정(2026-09-06): 스킵, 진행 안 함
