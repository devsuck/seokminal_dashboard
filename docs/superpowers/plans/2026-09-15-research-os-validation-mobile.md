# research-os/validation 모바일 카드 리디자인 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** `app/(console)/research-os/validation/page.tsx`의 3개 탭(검증 루프/위원회·프로덕션/인텔리전스+)에 `md:hidden` 모바일 카드 마크업을 추가한다. `hidden md:block` 데스크톱 브랜치는 바이트 단위로 무변경.

**Architecture:** 각 탭 컴포넌트(`ValidationTab`/`ProductionTab`/`IntelligencePlusTab`)는 이미 자체 데이터훅(`useState`+`useEffect` 또는 `useAbortableRun`)을 갖고 있다. 데스크톱 JSX를 `<div className="hidden md:block">`로 감싸고, 바로 아래 형제로 `<div className="md:hidden">` 모바일 카드 브랜치를 추가한다. 같은 컴포넌트 안이므로 데이터 변수(`data`/`err`/`vp`/`qp`/`ls` 등)를 그대로 재사용 — fetch 로직 복제 없음. 부모 `ValidationInner`도 동일 패턴으로 데스크톱 `TabBar`는 그대로 두고 모바일 전용 sticky 헤더+탭 pill바를 추가한다.

**Tech Stack:** Next.js (client component), Tailwind, `components/ui/ApPrimitives.tsx` (기존 라이트 카드 프리미티브, 신규 생성 없음), `lib/console-api.ts`(무변경, 재사용만).

**Spec:** `docs/superpowers/specs/2026-09-15-mobile-fintech-home-design.md`

## Global Constraints

- **프리미티브:** `ApPanel`, `ApPanelHead`, `ApDot`, `ApStatTile`, `ApBadge`, `ApSkeleton`, `ApSkeletonStatTile`, `ApSkeletonLines`, `ApMeter` — `components/ui/ApPrimitives.tsx`에서 import. `Card`/`CardHeader`(`components/ui/Card.tsx`)로 되돌리지 않는다, 신규 프리미티브 파일도 만들지 않는다 — investment-os 최종 리뷰에서 확정된 binding ruling, 재론 금지.
- **데스크톱 무변경:** 각 탭의 기존 return 블록(현재 `page.tsx` 라인 83-214/236-358/376-532)을 `<div className="hidden md:block">...</div>`로 감싸기만 한다. 내부 마크업/로직/data-fetching 훅은 한 글자도 고치지 않는다.
- **⚠️ "다크→라이트 전환" 아님 — 정정:** `app/(console)/layout.tsx`가 `(console)` 전 라우트를 `.rail-ap`로 감싸고(`app/globals.css:88-107`), 이 스코프는 뷰포트 무관하게 `--c-*`를 `--color-ap-*`로 리맵한다(2026-08-23 커밋 `f43a99f`). 즉 `Panel`/`PanelHead`/`Badge`/`StatTile`/`TabBar`/`PageHeader`가 쓰는 `--c-*` 토큰은 데스크톱에서도 이미 라이트 ap- 값으로 렌더된다 — "다크 터미널을 라이트로 바꾸는" 작업이 아니라 "이미 라이트인데 데스크톱 조밀 그리드로 짜인 레이아웃을 모바일 카드 스택으로 새로 짜는" 작업이다. 이 사실 때문에 **`STATE_TONE`/`EV_TONE`/`SEV_TONE`/`CONV_TONE`/`PRIO_TONE` 같은 기존 `var(--c-*)` 톤 매핑은 모바일 브랜치에서도 그대로 재사용 가능** — 별도 ap- 전용 톤 맵을 새로 만들지 않는다. (`EV_TONE`/`SEV_TONE`/`CONV_TONE`/`PRIO_TONE`는 `"pos"|"neg"|"warn"|"hud"|"mute"` 톤-키 값이라 `ApBadge`의 `tone` prop에 직접 꽂는다. `STATE_TONE`는 raw `"var(--c-hud)"` 문자열이라 `style={{color: STATE_TONE[s]}}` 인라인 스타일에 그대로 재사용한다.)
- **터치 타겟:** 인터랙티브 버튼/탭 최소 `h-11`(44px) — 탭 pill, 질문 입력창의 제출 버튼 포함.
- **타입 스케일 최소치:** 라벨 `text-[11px]`, 본문/숫자 `text-xs` — 모바일 브랜치에 `text-[9px]`를 쓰지 않는다(투자OS 라운드에서 fix-round로 잡았던 문제, 처음부터 준수).
- **모바일 루트 컨테이너:** 페이지가 이미 `.rail-ap` 안이므로 새 스코프 클래스 불필요. 각 탭의 `md:hidden` 루트는 `min-h-full`(investment-os 최종 fix에서 `min-h-screen`→`min-h-full`로 정정된 이유와 동일: 로딩 스켈레톤처럼 짧은 콘텐츠일 때도 뷰포트를 억지로 채우지 않음).
- **리스트 행(2차 배경):** `bg-ap-bg rounded-ap-md p-2.5` — investment-os에서 확립된 관례, `ApPanel`(=`bg-ap-surface`) 안에 얹는 보조 배경.
- **커밋:** main 직접 커밋(별도 브랜치 없음, investment-os와 동일 컨벤션). Task마다 커밋.
- **검증:** `npx tsc --noEmit`, `npm run build`, `npm test` — 각 태스크 리뷰와 최종 브랜치 리뷰에서 확인.

---

## Task 1: 모바일 셸(ValidationInner) + ValidationTab 모바일 카드

**Files:**
- Modify: `app/(console)/research-os/validation/page.tsx:19-215` (`ValidationInner` 함수, `ValidationTab` 함수)

**Interfaces:**
- Consumes: `TabBar`, `useAbortableRun`(`components/console/widgets.tsx`), `getValidationLoop`(`lib/console-api.ts`), `ApPanel`/`ApPanelHead`/`ApBadge`/`ApStatTile`/`ApSkeleton`/`ApSkeletonStatTile`/`ApSkeletonLines`(`components/ui/ApPrimitives.tsx`) — 새 import 필요.
- Produces: `ValidationInner`의 모바일 탭 pill 패턴(`h-11`, `bg-ap-brand` active/`bg-ap-surface` inactive)은 Task 2·3의 `ProductionTab`/`IntelligencePlusTab`에는 영향 없음(탭 전환 UI는 `ValidationInner` 한 곳에서만 렌더).

- [ ] **Step 1: import 추가**

`app/(console)/research-os/validation/page.tsx` 최상단 import 블록을 수정:
...
- [ ] **Step 2: `ValidationInner`에 모바일 셸 추가**
...
- [ ] **Step 3: `ValidationTab` — 데스크톱 브랜치를 `hidden md:block`으로 감싸기**

기존 (라인 83-214, `function ValidationTab() {` 본문 return):
...
교체 — 기존 내용은 `<div className="hidden md:block min-h-full">...</div>`로 한 글자도 바꾸지 않고 옮기고, 바로 아래 `<div className="md:hidden ...">` 형제를 추가:
...
Run: `npm run build`
Expected: 통과, `/research-os/validation` 라우트 포함.

- [ ] **Step 6: 커밋**

```bash
git add "app/(console)/research-os/validation/page.tsx"
git commit -m "feat(research-os/validation): 모바일 셸 + 검증 루프 탭 카드 마크업 추가"
```

---

## Task 2: ProductionTab 모바일 카드

**Files:**
- Modify: `app/(console)/research-os/validation/page.tsx` (`ProductionTab` 함수, Task 1 완료 후 라인 번호는 이동 — `// ---- production/page.tsx` 주석으로 찾는다)

**Interfaces:**
- Consumes: Task 1에서 import한 `ApPanel`/`ApPanelHead`/`ApBadge`/`ApStatTile`/`ApSkeleton*`, 기존 `useAbortableRun`, `evidenceText`, `SEV_TONE`/`CONV_TONE`/`CONV_LABEL`.
- Produces: 없음(리프 탭).

- [ ] **Step 1: `ProductionTab` — 데스크톱 브랜치를 `hidden md:block`으로 감싸고 모바일 브랜치 추가**

기존 return (`function ProductionTab() {` 본문):
...
```tsx
  return (
    <div className="min-h-full">
      ...
      <div className="p-5 space-y-5">
        {/* ...기존 전체(폼+4탭 stat+committee+debate+conviction+governance+production+portfolio+review)... */}
      </div>
    </div>
  );
```

교체 — 기존 전체를 `<div className="hidden md:block min-h-full">...</div>`로 그대로 옮기고, 아래 `<div className="md:hidden ...">` 형제 추가:
...
```bash
git add "app/(console)/research-os/validation/page.tsx"
git commit -m "feat(research-os/validation): 위원회·프로덕션 탭 모바일 카드 마크업 추가"
```

---

## Task 3: IntelligencePlusTab 모바일 카드

**Files:**
- Modify: `app/(console)/research-os/validation/page.tsx` (`IntelligencePlusTab` 함수 — `// ---- intelligence-plus/page.tsx` 주석으로 찾는다)

**Interfaces:**
- Consumes: Task 1에서 import한 Ap 프리미티브, 기존 `useAbortableRun`, `PRIO_TONE`.
- Produces: 없음(마지막 탭, 파일 최종 상태).

- [ ] **Step 1: `IntelligencePlusTab` — 데스크톱 브랜치를 `hidden md:block`으로 감싸고 모바일 브랜치 추가**

기존 return (`function IntelligencePlusTab() {` 본문):
...
```tsx
  return (
    <div className="min-h-full">
      ...
      <div className="p-5 space-y-5">
        {/* ...기존 전체(폼+4탭 stat+창의적가설+상시큐+실험우선순위+계획+생산성/회고+자율성검증)... */}
      </div>
    </div>
  );
```
...
Run: `npm test`
Expected: 기존 스위트 그린 유지.

- [ ] **Step 5: 커밋**

```bash
git add "app/(console)/research-os/validation/page.tsx"
git commit -m "feat(research-os/validation): 인텔리전스+ 탭 모바일 카드 마크업 추가"
```
