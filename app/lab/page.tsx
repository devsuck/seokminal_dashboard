...
import {
  getLabState, runLab, setLabAutopilot, getJarvisStatus, getJarvisDetail, getScanner,
...
function verdictStyle(s: string): string {
  ...
  if (s.startsWith("weak") || s.startsWith("underpowered")) return "border-info/40 text-info bg-info/10";
  return "border-neg/30 text-neg bg-neg/5";
}

function modeBadge(mode: string): { label: string; cls: string } {
  if (mode === "synthetic_demo") return { label: "합성 데모", cls: "border-info/40 text-info" };
  if (mode === "real_event") return { label: "실 KRX 검증", cls: "border-pos/40 text-pos" };
  if (mode === "blocked") return { label: "데이터 게이트", cls: "border-warn/40 text-warn" };
  return { label: "실검증", cls: "border-pos/40 text-pos" };
}
...
export default function LabPage() {
  ...
  async function onRunNext() { try { await runLab(); } catch { /* 폴링이 반영 */ } }
              ...
              <div key={sg.key} className="flex-1 flex items-center gap-2">
                <div
                  className={`flex-1 rounded-md border px-3 py-3 transition-all duration-300 ${
                    active
                      ? "border-accent bg-accent/10 shadow-[0_0_0_1px_var(--color-accent)]": done
                      ? "border-pos/30 bg-pos/5": "border-border bg-panel-2"}`}
                >
                  <div className="flex items-center gap-2">
                    <span className={`relative flex h-2 w-2 ${active ? "text-accent" : done ? "text-pos" : "text-text-3"}`}>
                      {active && <span className="absolute inline-flex h-full w-full rounded-full bg-current opacity-70 animate-ping" />}
                      <span className="relative inline-flex h-2 w-2 rounded-full bg-current" />
                    </span>
                    <span className={`text-sm font-semibold ${active ? "text-accent" : done ? "text-pos" : "text-text-2"}`}>{sg.label}</span>
                  </div>
                  <div className="text-[10px] uppercase tracking-widest text-text-3 mt-1 font-data">{sg.sub}</div>
                  {active && (
                    <div className="mt-2 h-1 rounded-full bg-panel overflow-hidden">
                      <div className={`h-full bg-accent rounded-full transition-all duration-300 ${widthClass(st?.progress ?? 0)}`} />
                    </div>
                  )}
                </div>
                {i < STAGES.length - 1 && (
                  <span className={`text-text-3 shrink-0 ${st && STAGES.findIndex(x => x.key === st.stage) >= i && st.stage ? "text-accent" : ""}`}>→</span>
                )}
              </div>
            );
          })}
...
function rtStyle(v: string | null): string {
  if (v === "CLEARED") return "border-pos/50 text-pos bg-pos/10";
  if (v === "REJECTED") return "border-neg/40 text-neg bg-neg/5";
...
function statusChip(f: ScannerFamily): { label: string; cls: string } {
  ...
  if (f.status === "백테스트 대기") return { label: "pull완·검증대기", cls: "border-info/40 text-info bg-info/10" };
  return { label: "pull 중/대기", cls: "border-text-3/30 text-text-3" };
}
...
const TERMINAL: { key: string; label: string; cls: string }[] = [
  { key: "blocked_by_data", label: "blocked", cls: "text-warn" },
  { key: "rejected", label: "rejected", cls: "text-neg" },
  { key: "retired", label: "retired", cls: "text-text-3" },
];

function auditColor(a: JarvisAuditRow): string {
  const s = `${a.result ?? ""}${a.execution_status ?? ""}${a.risk_status ?? ""}`.toLowerCase();
  if (a.permission_granted === false || s.includes("denied") || s.includes("blocked") || s.includes("rejected")) return "text-neg";
  if (s.includes("armed") || s.includes("committed") || s.includes("approved") || s.includes("paper_active")) return "text-pos";
  if (s.includes("simulated") || s.includes("allowed")) return "text-accent";
  return "text-text-3";
}
  ...
  return (
    <div className="bg-panel border border-border rounded-lg p-4 space-y-4">
        ...
        {LIFECYCLE.map((s, i) => (
          <div key={s.key} className="flex items-center gap-1">
            <div className={`rounded border px-2 py-1.5 text-center min-w-[64px] ${
              (counts[s.key] ?? 0) > 0 ? "border-accent/40 bg-accent/5" : "border-border bg-panel-2"}`}>
              <div className={`text-sm font-data ${(counts[s.key] ?? 0) > 0 ? "text-accent" : "text-text-3"}`}>{counts[s.key] ?? 0}</div>
              <div className="text-[9px] uppercase tracking-wide text-text-3">{s.label}</div>
            </div>
            {i < LIFECYCLE.length - 1 && <span className="text-text-3 text-xs">→</span>}
          </div>
        ))}
        <span className="text-text-3 mx-1">|</span>
        {TERMINAL.map(t => (
          <div key={t.key} className="rounded border border-border bg-panel-2 px-2 py-1.5 text-center min-w-[56px]">
            <div className={`text-sm font-data ${(counts[t.key] ?? 0) > 0 ? t.cls : "text-text-3"}`}>{counts[t.key] ?? 0}</div>
            <div className="text-[9px] uppercase tracking-wide text-text-3">{t.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Forward 배포 */}
        <div>
          <div className="text-xs uppercase tracking-wider text-text-3 font-semibold mb-1.5">Forward 배포 ({detail.deployments.length})</div>
          <div className="space-y-1 max-h-[160px] overflow-y-auto">
            {detail.deployments.length === 0 && <div className="text-xs text-text-3">배포 없음</div>}
            {detail.deployments.map(d => (
              <div key={d.strategy_id} className="flex items-center justify-between gap-2 text-xs bg-panel-2 border border-border rounded px-2 py-1">
                <span className="text-text-2 truncate">{d.strategy_id}</span>
                <span className="text-info font-data truncate shrink-0">{d.runner.split(":")[0].split(".").pop()}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-0.5 max-h-[160px] overflow-y-auto font-data text-[11px]">
            {detail.audit.slice(-14).reverse().map((a, i) => (
              <div key={i} className="flex gap-2">
                <span className="text-text-3 shrink-0 w-12 truncate">{a.layer ?? "-"}</span>
                <span className="text-text-2 shrink-0 w-24 truncate">{a.action ?? "-"}</span>
...
function CurrentCard({ st }: { st: LabState | null }) {
  ...
  const cell = (label: string, val: string, ready: boolean, color = "text-text-1") => (
    <div className="bg-panel-2 border border-border rounded px-3 py-2">
      <div className="text-[10px] uppercase tracking-wider text-text-3">{label}</div>
      <div className={`text-sm font-data mt-0.5 ${ready ? color : "text-text-3"} ${!ready && busy ? "animate-pulse" : ""}`}>
        {ready ? val : "···"}
      </div>
    </div>
  );
    ...
    <div className="bg-panel border border-border rounded-lg p-4">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] px-1.5 py-0.5 rounded border border-border text-text-2 font-data">{h.market}</span>
          <span className="text-[10px] px-1.5 py-0.5 rounded border border-border text-text-2">{h.family}</span>
        </div>
  ...
  return (
    <div className="bg-panel border border-border rounded-lg">
      ...
      <div className="h-[340px] overflow-y-auto px-4 py-3 font-data text-xs leading-relaxed">
        ...
        {log.map((l, i) => (
          <div key={i} className="flex gap-2">
            <span className="text-text-3 shrink-0">{l.ts}</span>
            <span className="text-text-3 shrink-0 uppercase w-14">{l.stage}</span>
...
function StatsRow({ st }: { st: LabState | null }) {
  ...
  return (
    <div className="grid grid-cols-5 gap-2">
      {item("검토", s.processed, "text-text-1")}
      {item("엣지", s.edges, "text-pos")}
      {item("기각", s.rejects, "text-neg")}
      {item("차단", s.blocked, "text-warn")}
  ...
  return (
    <div className="bg-panel border border-border rounded-lg">
      ...
      <div className="max-h-[200px] overflow-y-auto divide-y divide-border">
        ...
        {queue.map(h => (
          <div key={h.id} className={`px-4 py-2 flex items-center justify-between gap-2 ${h.id === currentId ? "bg-accent/5" : ""}`}>
            <span className="text-sm text-text-2 truncate">{h.name}</span>
            <span className="text-[10px] px-1.5 py-0.5 rounded border border-border text-text-3 shrink-0">{h.market}</span>
          </div>
        ))}
      </div>
    </div>
