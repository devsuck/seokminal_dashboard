...
function BuybackBotCard({ bot }: { bot: BuybackBot }) {
  return (
    <div className="bg-panel border border-pos/30 rounded-lg p-4 space-y-3">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <span className="text-base font-semibold text-text-1">Buyback 봇 · 페이퍼 실행</span>
          <span className="text-[10px] px-1.5 py-0.5 rounded border border-pos/40 text-pos bg-pos/10">검증된 v1 엣지</span>
        </div>
        <span className="text-[10px] text-warn font-data">live: {bot.live}</span>
      </div>
          ...
          <div className="space-y-0.5 max-h-[120px] overflow-y-auto">
            {bot.recent_closed.map((p, i) => (
              <div key={i} className="flex justify-between text-[11px]">
                ...
                <span className={`font-data shrink-0 ${p.pnl_pct >= 0 ? "text-pos" : "text-neg"}`}>{pct(p.pnl_pct)}</span>
              </div>
            ))}
          </div>
        </div>
...
function V2ShadowCard({ v2 }: { v2: V2Shadow }) {
  ...
  return (
    <div className="bg-panel border border-info/30 rounded-lg p-4 space-y-3">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2">
          <span className="text-base font-semibold text-text-1">buyback v2 · 레짐 필터</span>
          <span className="text-[10px] px-1.5 py-0.5 rounded border border-info/40 text-info bg-info/10">SHADOW (v1 동결)</span>
        </div>
        <span className="text-[10px] text-text-3 font-data">등록 {v2.frozen_date}</span>
      </div>
      ...
      {empty ? (
        ...
        <>
          <div className="mt-2 grid grid-cols-3 gap-1 text-xs font-data">
            ...
            <div className={`text-center ${pctColor(seg.v1_net)}`}>{pct(seg.v1_net)}</div>
            <div className="text-center text-text-2">{pct(seg.v1_winrate, 1)}</div>
            <div className="text-info">v2</div>
            <div className={`text-center ${pctColor(seg.v2_net)}`}>{pct(seg.v2_net)}</div>
            <div className="text-center text-text-1">{pct(seg.v2_winrate, 1)}</div>
          </div>
          <div className={`text-[10px] mt-1.5 ${seg.v2_improves ? "text-pos" : "text-text-3"}`}>
...
function PortfolioBookCard({ book }: { book: PortfolioBook }) {
  ...
  return (
    <div className="bg-panel border border-accent/30 rounded-lg p-4 space-y-3">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2">
          <span className="text-base font-semibold text-text-1">멀티엣지 포트폴리오 북</span>
          <span className="text-[10px] px-1.5 py-0.5 rounded border border-accent/40 text-accent bg-accent/10">A · 실제 굴릴 책</span>
        </div>
        <span className="text-[10px] text-text-3 font-data">{book.range ?? ""}</span>
      </div>
        ...
        {book.sleeves.map(s => (
          <div key={s.name} className="bg-panel-2 border border-border rounded px-3 py-2">
            ...
            <div className="flex gap-3 mt-1 text-xs font-data">
              <span className={pctColor(s.ann)}>연 {pct(s.ann)}</span>
              <span className="text-text-2">Sh {num(s.sharpe)}</span>
              <span className="text-neg">MDD {pct(s.mdd)}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      {c ? (
        <>
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className={`font-data font-semibold ${Math.abs(book.correlation ?? 0) < 0.3 ? "text-pos" : "text-warn"}`}>
          ...
          {monthly.length > 0 && (
            <div>
              <div className="flex items-center justify-between text-[10px] uppercase tracking-wider text-text-3 mb-1">
                ...
                <span className={`font-data ${(finalCum ?? 0) >= 0 ? "text-pos" : "text-neg"}`}>누적 {pct(finalCum)}</span>
              </div>
  ...
  return (
    <div className="bg-pos/5 border border-pos/25 rounded px-3 py-2">
      ...
      <div className="flex gap-3 mt-1 text-xs font-data">
        ...
        <span className={halved ? "text-pos font-semibold" : "text-neg"}>MDD {pct(s.mdd)}{halved ? " ↓" : ""}</span>
      </div>
    </div>
  );
}
...
function TaskCard({ task }: { task: LabTask }) {
  ...
  return (
    <div className="bg-panel border border-border rounded-lg p-4 space-y-3">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2">
          <span className="text-base font-semibold text-text-1">{task.strategy_id}</span>
          <span className={`text-[10px] px-1.5 py-0.5 rounded border ${statusStyle(task.status)}`}>{task.status}</span>
        </div>
        <span className="text-[10px] text-text-3 font-data">
          {task.runner ? task.runner.split(":")[0].split(".").pop() : "미배포"}
        </span>
      </div>
...
function MonthlyBar({ m, maxMag }: { m: LabTaskMonthly; maxMag: number }) {
  ...
  return (
    <div className="flex items-center gap-2 text-[11px]">
      ...
      <span className={`font-data w-14 text-right shrink-0 ${up ? "text-pos" : "text-neg"}`}>{pct(r)}</span>
