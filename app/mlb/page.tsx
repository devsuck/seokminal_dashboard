...
function CollectorStatus({ sys, restarting, onRestart }: {
  ...
  return (
    <Panel>
      <PanelHeader>수집기 상태</PanelHeader>
      <div className="flex items-center gap-3 px-4 py-3">
        <span className={`text-[9px] font-data font-bold px-1.5 py-0.5 ${running ? "bg-pos/20 text-pos" : "bg-neg/10 text-neg animate-blink"}`}>
          {running ? "ON" : "OFF"}
        </span>
        <span className="text-[11px] font-data text-text-2">
        ...
        {!running && sys && (
          <button
            onClick={onRestart}
            disabled={restarting}
            className="text-[10px] px-2 py-1 border border-neg/50 text-neg bg-neg/15 font-data font-bold hover:bg-neg/25 disabled:opacity-40"
          >
            {restarting ? "재시작중…" : "재시작"}
          </button>
        )}
...
export default function MlbPage() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-text-1">MLB Specialist Consensus</h1>
      ...
      {!edge ? <div className="text-text-3 text-xs">로딩 중…</div>
        : !rep ? <div className="text-text-3 text-xs">{edge.warming ? "백그라운드 계산 중 — 잠시 후 표시됩니다…" : "검증 결과 없음"}</div>
