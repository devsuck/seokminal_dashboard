...
function Metric({ label, value, cls }: { label: string; value: string; cls?: string }) {
  return (
    <div className="bg-panel border border-border rounded-lg p-3">
      ...
      <div className={`text-lg font-data font-bold mt-1 ${cls ?? "text-text-1"}`}>{value}</div>
    </div>
  );
}
...
function EquityChart({ data }: { data: PerfSummary }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ height: "300px" }}>
      {/* baseline (시작 equity) */}
      <line x1={P} x2={W - P} y1={y(eqs[0])} y2={y(eqs[0])} stroke="var(--color-border)" strokeDasharray="3 3" />
      {/* benchmark */}
      {bens.length > 0 && (
        <polyline points={line(pts.map(p => p.benchmark))} fill="none" stroke="var(--color-text-3)" strokeWidth="1.5" strokeDasharray="4 3" />
      )}
      {/* strategy */}
      <polyline points={line(eqs)} fill="none" stroke="var(--color-accent)" strokeWidth="2" />
      {/* y labels */}
      <text x={4} y={y(yMax) + 4} fill="var(--color-text-3)" fontSize="10">{Math.round(yMax).toLocaleString()}</text>
      <text x={4} y={y(yMin) + 4} fill="var(--color-text-3)" fontSize="10">{Math.round(yMin).toLocaleString()}</text>
      {/* x labels (start/end) */}
      <text x={P} y={H - 8} fill="var(--color-text-3)" fontSize="10">{pts[0].date}</text>
      <text x={W - P} y={H - 8} fill="var(--color-text-3)" fontSize="10" textAnchor="end">{pts[pts.length - 1].date}</text>
    </svg>
  );
}
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-start justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-text-1 text-lg font-semibold">성과 추적</h1>
          <p className="text-text-3 text-sm mt-0.5">
            페이퍼 계좌(Alpaca) equity 곡선 · 수익률/MDD/Sharpe · <span className="text-text-2">SPY 매수보유</span> 벤치마크 대비. 전략이 그냥 지수 든 것보다 나은지 판단.
          </p>
        </div>
        <div className="flex rounded overflow-hidden border border-border">
          {PERIODS.map(p => (
            <button key={p} onClick={() => setPeriod(p)}
              className={`px-3 py-1.5 text-xs ${period === p ? "bg-accent/15 text-accent" : "bg-panel-2 text-text-3 hover:text-text-2"}`}>
              {PERIOD_LABEL[p]}
            </button>
          ))}
        </div>
      ...
      {error ? <EmptyState message="성과 로드 실패" hint={error} />
        : loading ? <LoadingState message="성과 계산 중…" />
          ...
          <>
            ...
            <div className="bg-panel border border-border rounded-lg p-4">
              <div className="flex items-center gap-4 mb-2 text-[11px]">
                <span className="flex items-center gap-1.5"><span className="w-4 h-0.5 bg-accent inline-block" /> 내 전략</span>
                <span className="flex items-center gap-1.5"><span className="w-4 h-0.5 bg-text-3 inline-block" style={{ borderTop: "1px dashed" }} /> SPY 매수보유</span>
                <span className="ml-auto text-text-3 font-data">
                  ${data.start_equity.toLocaleString()} → ${data.end_equity.toLocaleString()}
                </span>
              </div>
              {data.points.length < 2 ? (
                <EmptyState message="거래 이력 부족" hint="페이퍼 매매 후 곡선이 그려집니다" />
