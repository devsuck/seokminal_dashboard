...
const GATE_LABEL: Record<string, string> = {
  walk_forward: "Walk-Forward", monte_carlo: "Monte Carlo", bh_fdr: "BH-FDR (multiple testing)",
  cost_stress: "Cost Stress", redteam: "Red Team Audit",
};
...
export default function ValidationReport() {
  ...
  return (
    <div className="min-h-full">
      <PageHeader kicker="QUANT LAB" title="Strategy Validation Report"
        right={<Badge tone={allAgree ? "pos" : "warn"}>{allAgree ? "REDTEAM ✓ CONSENSUS" : "REVIEW"}</Badge>} />
        ...
        <StateBlock loading={loading} err={err}>
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.2fr] gap-5">
            ...
            <Panel grid className="overflow-hidden">
              <PanelHead kicker="ROBUSTNESS" title="Validation Gates" />
            ...
            <Panel className="overflow-hidden">
              <PanelHead kicker="ADVERSARIAL" title="Red Team vs Human" right={<span className="c-num text-[11px] text-[var(--c-hud)]">{agree}/{n} agree</span>} />
              <div className="overflow-x-auto">
                <table className="w-full border-collapse">
                  <thead><tr className="border-b border-[var(--c-border)]">
                    {["Strategy", "Human", "Red Team", "Match"].map((h) => <th key={h} className="text-[9px] font-semibold tracking-[0.14em] text-[var(--c-text-3)] uppercase px-3 py-2 text-left">{h}</th>)}
                  </tr></thead>
                  <tbody>
                    {(rt?.rows ?? []).map((row, i) => {
          ...
          <Panel className="overflow-hidden">
            <PanelHead kicker="COHORT" title="Experiment Status Distribution" />
