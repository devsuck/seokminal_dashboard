  ...
  return (
    <div className="min-h-full">
      <PageHeader kicker="EXECUTION" title="Pipeline Monitor"
        right={<span className="text-[10px] c-num text-[var(--c-text-3)]">{data?.proposals ?? 0} prop · {data?.approvals ?? 0} appr</span>} />
      <div className="p-5 space-y-5 max-w-[1200px]">
        <StateBlock loading={loading} err={err}>
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
            <StatTile label="Capital" value={data?.capital.capital != null ? `$${(data.capital.capital / 1e6).toFixed(1)}M` : "—"} tone="hud" accent="hud" />
            <StatTile label="Exposure" value={(data?.capital.exposure_pct ?? 0).toFixed(1)} unit="%" accent="info" />
            <StatTile label="Positions" value={data?.capital.n_positions ?? 0} accent="warn" />
          </div>
          <Panel grid className="overflow-hidden">
            <PanelHead kicker="P8" title="Execution Pipeline (7.4 → 8.7)" />
            <div className="p-3">
              {(data?.stages ?? []).map((s, i) => {
                ...
                return (
                  <div key={s.key} className="flex items-center gap-3 py-2.5 px-1 border-b border-[var(--c-border)] last:border-0">
                    ...
                    <span className="c-num text-[10.5px] text-[var(--c-text-3)]">{Object.entries(s.by_status).map(([k, v]) => `${k}:${v}`).join(" ") || "idle"}</span>
