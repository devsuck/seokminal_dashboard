...
export default function LiveIntelligence() {
  ...
  return (
    <div className="min-h-full">
      <PageHeader kicker="P111-120" title="Live Intelligence"
        ...
        {data && (
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              <StatTile label="Providers" value={ds?.count ?? 0} sub={`${ds?.available_count ?? 0} available`} tone="hud" />
              <StatTile label="Market Feed" value={data.market_feed.length} sub="events" />
              <StatTile label="Research Queue" value={data.research_queue_count} sub="candidates" tone="pos" />
              <StatTile label="Data Health" value={dh?.overall_status ?? "—"}
                sub={`${dh?.api_availability?.available ?? 0}/${dh?.api_availability?.total ?? 0} APIs · ${dh?.issue_count ?? 0} issues`}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {/* 1. Data Sources */}
              <Panel>
                <PanelHead kicker="1 · Data Sources" title="Provider Status"
                ...
                <div className="p-4 space-y-1.5 max-h-[440px] overflow-y-auto">
                  {(ds?.providers ?? []).map((p) => (
                    <div key={p.name} className="c-panel-2 p-2.5 flex items-center gap-2">
                      ...
                      <Badge tone={p.available ? "pos" : "mute"}>{p.status === "available" ? "OK" : "CFG"}</Badge>
                    </div>
                  ))}
                </div>
              </Panel>
              ...
              {/* 4. Data Health */}
              <Panel>
                <PanelHead kicker="4 · Data Health" title="API / Data Quality"
                  right={dh && <Badge tone={HEALTH_TONE[dh.overall_status] ?? "mute"}>{dh.overall_status}</Badge>} />
                <div className="p-4 space-y-3">
                  ...
                  {(dh?.api_availability?.unavailable ?? []).length > 0 && (
                    <div className="text-[10px] text-[var(--c-text-3)]">Not configured: {dh?.api_availability?.unavailable.join(", ")}</div>
                  )}
                </div>
              </Panel>
            </div>
            ...
            {/* 2. Market Feed */}
            <Panel>
              <PanelHead kicker="2 · Market Feed" title="Events · News · Earnings"
                right={<>{data.is_demo && <span className="text-[9px] text-[var(--c-text-3)] uppercase mr-2">demo</span>}<Badge tone="hud">{data.market_feed.length}</Badge></>} />
            ...
            {/* 3. Research Queue */}
            <Panel>
              <PanelHead kicker="3 · Research Queue" title="Generated Candidates"
              ...
              <div className="p-4 space-y-2">
                ...
                {data.dropped_duplicates > 0 && <div className="text-[9px] text-[var(--c-text-3)]">중복 방지: {data.dropped_duplicates} dropped</div>}
              </div>
            </Panel>
