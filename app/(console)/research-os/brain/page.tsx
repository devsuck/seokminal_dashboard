...
export default function ResearchBrain() {
  ...
  return (
    <div className="min-h-full">
      <PageHeader kicker="P131-140" title="Research Brain"
      ...
      <div className="p-5 space-y-5">
        <form onSubmit={(e) => { e.preventDefault(); run(topic); }} className="flex gap-2">
          <input value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="주제 필터 (예: momentum, TSMC)…"
          ...
          <button type="submit" className="px-4 h-10 text-[11px] font-semibold uppercase text-[var(--c-hud)] border border-[color-mix(in_srgb,var(--c-hud)_40%,transparent)] bg-[color-mix(in_srgb,var(--c-hud)_10%,transparent)] cursor-pointer">Recall</button>
        </form>
        ...
        {data && (
          <>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              <StatTile label="Graph Nodes" value={data.knowledge_graph.node_count} sub="knowledge" tone="hud" />
              <StatTile label="Lessons" value={data.lessons.length} sub="accumulated" tone="pos" />
              <StatTile label="Conflicts" value={data.conflicts.length} sub="contradictions" tone={data.conflicts.length ? "warn" : "pos"} />
              <StatTile label="Knowledge Health" value={kh?.grade ?? "—"} sub={`${kh?.health_score ?? 0} / 100`} tone={HEALTH_TONE[kh?.grade ?? ""] === "neg" ? "neg" : HEALTH_TONE[kh?.grade ?? ""] === "warn" ? "warn" : "pos"} />
            </div>

            {/* Knowledge Graph */}
            <Panel>
              <PanelHead kicker="Knowledge Graph" title="Question → Hypothesis → Experiment → Result → Lesson"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {/* Failure Patterns */}
              <Panel>
                <PanelHead kicker="Failure Patterns" title="What Failed" right={<Badge tone={data.failure_patterns.total_failures ? "neg" : "pos"}>{data.failure_patterns.total_failures}</Badge>} />
                <div className="p-4 space-y-1.5">
                  ...
                  {data.failure_patterns.top_category && <div className="text-[10px] text-[var(--c-warn)] pt-1">Top: {data.failure_patterns.top_category}</div>}
                </div>
              </Panel>

              {/* Conflicts */}
              <Panel>
                <PanelHead kicker="Conflicts" title="Contradicting Conclusions" right={<Badge tone={data.conflicts.length ? "warn" : "pos"}>{data.conflicts.length}</Badge>} />
                <div className="p-4 space-y-2">
                  {data.conflicts.length === 0 && <div className="text-[11px] text-[var(--c-text-3)]">같은 주제에서 works vs fails 모순이 감지되면 여기에 표시됩니다.</div>}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <MemoryList title="Strategy Memory" kicker="Strategies" nodes={data.strategy_memory} tone="var(--c-emerald)" />
              <MemoryList title="Company Memory" kicker="Companies" nodes={data.company_memory} tone="var(--c-blue)" />
              <MemoryList title="Lessons" kicker="Learned" nodes={data.lessons} tone="var(--c-pos)" />
            </div>
