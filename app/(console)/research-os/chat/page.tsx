      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          ...
          {SUGGESTIONS.map((s) => (
            <button key={s} onClick={() => { setQ(s); run(s); }} disabled={loading}
              className="px-2.5 py-1 text-[11px] text-[var(--c-text-2)] border border-[var(--c-border)] hover:border-[var(--c-hud)] hover:text-[var(--c-hud)] bg-transparent cursor-pointer disabled:opacity-40 transition-colors">
              {s}
            </button>
          ))}
        </div>
        {err && <div className="c-panel p-4 text-[13px] text-[var(--c-neg)] mb-4">백엔드 연결 실패: {err}</div>}
        {loading && !turn && (
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-2">
                    <div>
                      ...
                      {(turn.memo.supporting_arguments ?? []).map((a, i) => (
                        <div key={i} className="text-[11px] text-[var(--c-text-2)] mb-1">· <b className="text-[var(--c-text-1)]">{a.lens}</b> {a.rationale}</div>
                      ))}
                      {(turn.memo.supporting_arguments ?? []).length === 0 && <div className="text-[11px] text-[var(--c-text-3)]">—</div>}
                    </div>
                    <div>
                      ...
                      {(turn.memo.counter_arguments ?? []).length === 0 && <div className="text-[11px] text-[var(--c-text-3)]">—</div>}
                    </div>
                  </div>
                </div>
              </Panel>
              ...
              <Panel>
                ...
                <div className="p-4 space-y-1">
                  ...
                  {(turn.ev.references_experiments ?? []).map((r, i) => (
                    <div key={i} className="text-[11px] c-num text-[var(--c-text-2)]">· {r}</div>
                  ))}
        ...
        {!turn && !loading && (
          <div className="c-panel p-8 text-center text-[13px] text-[var(--c-text-3)]">
            ...
            <span className="text-[11px]">분석·회상만 — 투자 결정·집행은 사람이 합니다.</span>
          </div>
        )}
      </div>
    </div>
