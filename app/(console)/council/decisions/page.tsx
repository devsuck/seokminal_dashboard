...
export default function CouncilDecisions() {
  ...
  return (
    <div className="min-h-full">
      <PageHeader kicker="AI COUNCIL" title="Decision Feed"
      ...
      <div className="p-5 max-w-[1000px]">
        <StateBlock loading={loading} err={err} empty={!rows.length} emptyNote="포트폴리오 의사결정 엔진이 결정을 생성하면 여기에 표시됩니다.">
          <Panel>
            <PanelHead kicker="STREAM" title="Latest Decisions" right={<span className="text-[10px] c-num text-[var(--c-text-3)]">{data?.count ?? 0}</span>} />
            <ul className="p-2">
              {[...rows].reverse().map((d, i) => (
                <li key={i} className="flex items-start gap-3 px-2 py-2.5 border-b border-[var(--c-border)] last:border-0">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-[12px] text-[var(--c-text-1)]">{String(d.action ?? d.decision ?? d.event ?? d.message ?? "decision")}</div>
