...
function statusStyle(s: string): string {
  if (s === "paper_active") return "border-ap-up/40 text-ap-up bg-ap-up/10";
  if (s.startsWith("paper_candidate")) return "border-ap-brand/40 text-ap-brand bg-ap-brand/10";
  return "border-ap-line text-ap-ink-2";
}
...
export default function TasksTab() {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-5">
      ...
      {loading && <div className="text-ap-ink-3 text-sm">불러오는 중…</div>}
      {err && <div className="text-xs text-ap-down border border-ap-down/30 rounded px-3 py-2">오류: {err}</div>}
    ...
    <ApPanel>
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            <div className="text-ap-body uppercase tracking-wider text-ap-ink-3 mb-1">보유중 (다음 청산 대기)</div>
            <div className="space-y-0.5 max-h-[120px] overflow-y-auto">
              {bot.open_positions.length === 0 && <div className="text-xs text-ap-ink-3">없음</div>}
              {bot.open_positions.map((p, i) => (
                <div key={i} className="flex justify-between text-ap-body">
                  <span className="text-ap-ink-2 truncate">{p.corp || p.code}</span>
                  <span className="text-ap-ink-3 font-data shrink-0">진입 {p.entry_date}</span>
                </div>
              ))}
            </div>
          </div>
          <div>
            <div className="text-ap-body uppercase tracking-wider text-ap-ink-3 mb-1">최근 청산</div>
            <div className="space-y-0.5 max-h-[120px] overflow-y-auto">
              {bot.recent_closed.map((p, i) => (
                <div key={i} className="flex justify-between text-ap-body">
                  <span className="text-ap-ink-2 truncate">{p.corp}</span>
                  <span className={`font-data px-1 font-bold shrink-0 ${p.pnl_pct >= 0 ? "bg-ap-up/20 text-ap-up" : "bg-ap-down/20 text-ap-down"}`}>{pct(p.pnl_pct)}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="text-ap-body text-ap-ink-3 border-t border-ap-line pt-2">
...
function V2ShadowCard({ v2 }: { v2: V2Shadow }) {
  ...
  return (
    <ApPanel>
      ...
      <div className="p-4 space-y-3">
        <p className="text-ap-body text-ap-ink-3">{v2.rule}</p>
        ...
        <div className="text-ap-body text-ap-caution border-t border-ap-line pt-2">
          ⚠ {v2.discipline}
...
function V2SegBox({ title, seg, sub, forward }: { title: string; seg: V2Seg; sub: string; forward?: boolean }) {
  ...
  return (
    <div className={`rounded border px-3 py-2 ${forward ? "border-ap-note/25 bg-ap-note/5" : "border-ap-line bg-ap-bg"}`}>
      <div className="text-xs font-medium text-ap-ink-2">{title}</div>
      {empty ? (
        <div className="text-xs text-ap-ink-3 mt-2">{sub}</div>
      ) : (
        <>
          <div className="mt-2 grid grid-cols-3 gap-1 text-xs font-data">
            <div />
            <div className="text-ap-ink-3 text-center">net</div>
            <div className="text-ap-ink-3 text-center">승률</div>
            <div className="text-ap-ink-3">v1</div>
            <div className={`text-center px-1 font-bold ${pctBgColor(seg.v1_net)}`}>{pct(seg.v1_net)}</div>
            <div className="text-center text-ap-ink-2">{pct(seg.v1_winrate, 1)}</div>
            <div className="text-ap-note">v2</div>
          ...
          <div className={`text-ap-body mt-1.5 px-1 ${seg.v2_improves ? "font-bold bg-ap-up/20 text-ap-up" : "text-ap-ink-3"}`}>
          ...
          {book.sleeves.map(s => (
            <div key={s.name} className="bg-ap-bg border border-ap-line rounded px-3 py-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-ap-body text-ap-ink-3">n{s.n}</span>
              </div>
              <div className="flex gap-3 mt-1 text-xs font-data">
                <span className={`px-1 font-bold ${pctBgColor(s.ann)}`}>연 {pct(s.ann)}</span>
                <span className="text-ap-ink-2">Sh {num(s.sharpe)}</span>
                <span className="px-1 font-bold bg-ap-down/20 text-ap-down">MDD {pct(s.mdd)}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        {c ? (
          <>
            <div className="flex items-center gap-2 text-xs">
              <span className="text-ap-ink-3">상관</span>
              <span className={`font-data font-semibold px-1 ${Math.abs(book.correlation ?? 0) < 0.3 ? "bg-ap-up/20 text-ap-up" : "bg-ap-caution/20 text-ap-caution"}`}>
                {typeof book.correlation === "number" ? book.correlation.toFixed(2) : "—"}
              </span>
              <span className="text-ap-ink-3">{Math.abs(book.correlation ?? 0) < 0.3 ? "(무상관 → 분산이득)" : ""}</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            {monthly.length > 0 && (
              <div>
                <div className="flex items-center justify-between text-ap-body uppercase tracking-wider text-ap-ink-3 mb-1">
          ...
          <div className="text-xs text-ap-ink-3">겹치는 기간 부족 — 조합 통계 대기(슬리브 개별만).</div>
        )}

        {/* live-readiness 제약(②) */}
        {book.constraints && (
          <div className="border-t border-ap-line pt-2 space-y-1">
            <div className="text-ap-body uppercase tracking-wider text-ap-ink-3">live-readiness 제약</div>
            {Object.entries(book.constraints).map(([k, c]) => (
              <div key={k} className="flex items-center gap-2 text-ap-body flex-wrap">
                <span className="text-ap-ink-2 font-medium w-16">{k}</span>
                <span className="text-ap-ink-3">규모 <span className="text-ap-ink-1">{c.scale}</span></span>
                <span className="text-ap-ink-3">수용력 <span className="text-ap-ink-1">{c.capacity}</span></span>
                <span className="text-ap-caution">{c.timing}</span>
              </div>
            ))}
          </div>
        )}

        <div className="text-ap-body text-ap-ink-3 border-t border-ap-line pt-2">{book.note}</div>
      </div>
    </ApPanel>
  );
}
...
function ComboBox({ label, s, bestMdd }: { label: string; s: { ann: number; sharpe: number; mdd: number }; bestMdd: number }) {
  ...
  return (
    <div className="bg-ap-up/5 border border-ap-up/25 rounded px-3 py-2">
      <div className="text-xs text-ap-ink-2">{label}</div>
      <div className="flex gap-3 mt-1 text-xs font-data">
        <span className={`px-1 font-bold ${pctBgColor(s.ann)}`}>연 {pct(s.ann)}</span>
        <span className="text-ap-up font-semibold">Sh {num(s.sharpe)}</span>
        <span className={`px-1 font-bold ${halved ? "bg-ap-up/20 text-ap-up" : "bg-ap-down/20 text-ap-down"}`}>MDD {pct(s.mdd)}{halved ? " ↓" : ""}</span>
      </div>
    </div>
  );
}
...
function CumBar({ m, maxCum }: { m: BookMonthly; maxCum: number }) {
  ...
  return (
    <div className="flex-1 flex flex-col justify-end h-full" title={`${m.period}: 누적 ${(m.cum * 100).toFixed(1)}% / 월 ${(m.combined * 100).toFixed(1)}%`}>
      <div className={`w-full rounded-sm ${up ? "bg-ap-up/60" : "bg-ap-down/60"} ${heightClass(h)}`} />
    </div>
  );
}

function pctBgColor(n: number | null | undefined): string {
  return typeof n === "number" ? (n >= 0 ? "bg-ap-up/20 text-ap-up" : "bg-ap-down/20 text-ap-down") : "text-ap-ink-3";
}
    ...
    <ApPanel>
      ...
      <div className="p-4 space-y-3">
        ...
        {fw?.stats_warming && !fw.error && (
          <div className="text-ap-body text-ap-note">통계 계산 중(서버 배경 워밍) — 잠시 후 채워짐. 규칙은 아래 표시.</div>
        )}
        ...
        {fw && !fw.error && (
          <>
            ...
            <div className="flex flex-wrap gap-x-5 gap-y-1 text-xs">
              <span className="text-ap-ink-3">진입 <span className="text-ap-ink-1">{fw.entry ?? "—"}</span></span>
              <span className="text-ap-ink-3">청산 <span className="text-ap-ink-1">{fw.exit ?? "—"}</span></span>
              {typeof fw.cost_bps === "number" && <span className="text-ap-ink-3">비용 <span className="text-ap-ink-1 font-data">{fw.cost_bps}bps</span></span>}
            </div>
            ...
            {recent.length > 0 && (
              <div>
                <div className="text-ap-body uppercase tracking-wider text-ap-ink-3 mb-1">월별 수익 (최근 {recent.length})</div>
...
function Stat({ label, val, pos }: { label: string; val: string; pos?: boolean }) {
  ...
  return (
    <div className="bg-ap-bg border border-ap-line rounded px-2.5 py-1.5">
      <div className="text-ap-body uppercase tracking-wider text-ap-ink-3">{label}</div>
...
function MonthlyBar({ m, maxMag }: { m: LabTaskMonthly; maxMag: number }) {
  ...
  return (
    <div className="flex items-center gap-2 text-ap-body">
      <span className="text-ap-ink-3 font-data w-16 shrink-0">{m.period}</span>
      <div className="flex-1 h-3 bg-ap-bg rounded-sm overflow-hidden flex">
        <div className={`h-full rounded-sm ${up ? "bg-ap-up/60" : "bg-ap-down/60"} ${barW(mag)}`} />
      </div>
      <span className={`font-data w-14 text-right shrink-0 px-1 font-bold ${up ? "bg-ap-up/20 text-ap-up" : "bg-ap-down/20 text-ap-down"}`}>{pct(r)}</span>
      {typeof m.n === "number" && <span className="text-ap-ink-3 w-8 text-right shrink-0">n{m.n}</span>}
    </div>
  );
}
