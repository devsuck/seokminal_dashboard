...
const ALLOC_COLORS = ["bg-ap-brand/70", "bg-ap-note/70", "bg-ap-up/70", "bg-ap-caution/70", "bg-ap-down/60"];
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-5">
      ...
      {bot && (
        <Link href="/lab/execution"
          ...
          {bot.cum_paper_pnl != null && (
            <span className={`text-[11px] font-data px-1 font-bold ${bot.cum_paper_pnl >= 0 ? "bg-ap-up/20 text-ap-up" : "bg-ap-down/20 text-ap-down"}`}>
              누적 {bot.cum_paper_pnl >= 0 ? "+" : ""}{bot.cum_paper_pnl.toFixed(2)}%
            </span>
          )}
          {exec?.arm_decision && (
            <span className={`text-[10px] px-1.5 py-0.5 rounded border font-data ${
              exec.arm_decision.decision === "GO" ? "border-ap-up/50 text-ap-up bg-ap-up/10" :
              exec.arm_decision.decision === "KILL" ? "border-ap-down/50 text-ap-down bg-ap-down/10 animate-blink" :
              "border-ap-note/40 text-ap-note bg-ap-note/10"}`}>
              ARM {exec.arm_decision.decision}
            </span>
          )}
          {paperTasks && paperTasks.length > 0 && (
            <span className="text-[11px] font-data text-ap-ink-3">
              페이퍼 전략 {paperTasks.length} · {paperTasks.map(t => t.strategy_id.replace(/^futures_|^kr_dart_/, "")).join(" · ")}
            </span>
          )}
          <span className="ml-auto text-[11px] text-ap-brand shrink-0">집행 콘솔 →</span>
        </Link>
      )}
      ...
      {rows && rows.length === 0 && (
        <div className="bg-ap-surface border border-ap-line rounded-lg p-6 text-center text-ap-ink-3 text-sm">
          운용 중인 AI 없음 — <Link href="/agents" className="text-ap-brand no-underline">에이전트</Link>에서 생성.
        </div>
      )}
      ...
      {groups.some(g => g.alloc > 0) && (
        <Card>
          <CardHeader>AI별 자본 배분</CardHeader>
          <div className="p-3 space-y-3">
            {groups.filter(g => g.alloc > 0).map(g => (
              <div key={g.cur} className="space-y-1.5">
                <div className="text-[10px] uppercase tracking-wider text-ap-ink-3">{g.cur} · {amt(g.alloc)}</div>
                <div className="flex h-4 rounded overflow-hidden border border-ap-line">
                  {g.rows.map((r, i) => {
                ...
                <div className="flex flex-wrap gap-x-3 gap-y-1">
                  {g.rows.map((r, i) => {
                    const w = ((r.perf?.alloc ?? r.agent.account_alloc ?? 0) / g.alloc) * 100;
                    return (
                      <span key={r.agent.id} className="flex items-center gap-1.5 text-[11px] text-ap-ink-3">
                        <span className={`inline-block w-2 h-2 rounded-sm ${ALLOC_COLORS[i % ALLOC_COLORS.length]}`} />
                        <span className="text-ap-ink-2">{r.agent.name}</span>
      ...
      {pnlAll && (
        <Card>
          <CardHeader>독립봇 실현손익</CardHeader>
          <div className="p-3 space-y-1.5">
            {pnlAll.bots.map(b => (
              <div key={b.id} className="flex items-center justify-between text-xs">
                <span className="text-ap-ink-2">{b.name}{b.note && <span className="text-ap-ink-3"> ({b.note})</span>}</span>
                <span className={`font-data px-1 font-bold ${b.realized_pnl === null ? "text-ap-ink-3" : b.realized_pnl >= 0 ? "bg-ap-up/20 text-ap-up" : "bg-ap-down/20 text-ap-down"}`}>
                  {b.realized_pnl === null ? "—" : `${b.realized_pnl >= 0 ? "+" : "-"}$${amt(Math.abs(b.realized_pnl))}`}
                </span>
              </div>
            ))}
            {/* 서버의 grand_total은 KRW 에이전트 손익까지 더해서(=통화 혼합) 의미가 없어 쓰지 않음 */}
            <div className="flex items-center justify-between text-xs pt-1.5 border-t border-ap-line">
              <span className="text-ap-ink-1 font-semibold">독립봇 합계 ($)</span>
              <span className={`font-data px-1 font-bold ${pnlAll.bots_totals.realized_pnl >= 0 ? "bg-ap-up/20 text-ap-up" : "bg-ap-down/20 text-ap-down"}`}>
      ...
      <div className="space-y-2">
        {rows?.map(r => {
          ...
          return (
            <div key={r.agent.id} className="bg-ap-surface border border-ap-line rounded-lg">
              <button onClick={() => setOpenId(open ? null : r.agent.id)}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-ap-ink-1 truncate">{r.agent.name}</span>
                    <span className="text-[10px] px-1.5 py-0.5 rounded border border-ap-line text-ap-ink-3">{r.agent.market}</span>
                    {r.agent.paper && <span className="text-[10px] px-1.5 py-0.5 rounded border border-ap-note/40 text-ap-note">페이퍼</span>}
                    {r.agent.validated === false && (
                      <span className="text-[10px] px-1.5 py-0.5 rounded border border-ap-caution/40 text-ap-caution bg-ap-caution/10"
                        title={r.agent.validation_reason ?? "registry 미등록 전략"}>미검증</span>
                    )}
                  </div>
                  <div className="text-[11px] text-ap-ink-3">배분 {currencyOf(r.agent.market)}{amt(p?.alloc ?? r.agent.account_alloc)} · 자율 Lv{r.agent.autonomy}</div>
                </div>
                <div className="text-right shrink-0">
                  <div className={`text-sm font-data px-1 font-bold inline-block ${(p?.return_pct ?? 0) >= 0 ? "bg-ap-up/20 text-ap-up" : "bg-ap-down/20 text-ap-down"}`}>{pct(p?.return_pct)}</div>
                  <div className="text-[11px] text-ap-ink-3 font-data">{won(p?.total_pnl)} · {p?.trades?.length ?? 0}건</div>
                </div>
                <span className={`text-ap-ink-3 transition-transform ${open ? "rotate-90" : ""}`}>›</span>
              </button>

              {open && p && (
                <div className="px-4 pb-4 border-t border-ap-line pt-3 space-y-3">
                  ...
                  {p.open_positions.length > 0 && (
                    <div>
                      <div className="text-[10px] uppercase tracking-wider text-ap-ink-3 mb-1">보유 포지션</div>
                      {p.open_positions.map((o, i) => (
                        <div key={i} className="flex items-center justify-between text-[11px] font-data py-0.5">
                          <span className="text-ap-brand no-underline">{o.symbol}</span>
                          <span className="text-ap-ink-3">{o.qty}주 @ {o.avg_price}</span>
                          <span className={`px-1 font-bold ${(o.unrealized_pnl ?? 0) >= 0 ? "bg-ap-up/20 text-ap-up" : "bg-ap-down/20 text-ap-down"}`}>{won(o.unrealized_pnl)}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  <div>
                    <div className="text-[10px] uppercase tracking-wider text-ap-ink-3 mb-1">최근 매매기록 ({p.trades.length})</div>
                    <div className="space-y-0.5 max-h-[200px] overflow-y-auto">
                      {p.trades.slice(-15).reverse().map((t, i) => (
                        <div key={i} className="flex items-center gap-2 text-[11px] font-data">
                          <span className="text-ap-ink-3 w-24 shrink-0 truncate">{t.ts?.slice(5, 16) ?? "—"}</span>
                          <span className={`w-8 shrink-0 px-1 font-bold ${t.side === "buy" ? "bg-ap-up/20 text-ap-up" : "bg-ap-down/20 text-ap-down"}`}>{t.side === "buy" ? "매수" : "매도"}</span>
                          <span className="text-ap-brand no-underline w-14 shrink-0 truncate">{t.symbol}</span>
                          <span className="text-ap-ink-3">{t.qty}@{t.price}</span>
                          {t.realized_pnl !== null && <span className={`ml-auto px-1 font-bold ${t.realized_pnl >= 0 ? "bg-ap-up/20 text-ap-up" : "bg-ap-down/20 text-ap-down"}`}>{won(t.realized_pnl)}</span>}
                        </div>
                      ))}
                      {p.trades.length === 0 && <div className="text-[11px] text-ap-ink-3">아직 매매 없음</div>}
                    </div>
                  </div>
                  <Link href={`/agents`} className="inline-block text-xs text-ap-brand border border-ap-brand/30 rounded px-2.5 py-1 no-underline hover:bg-ap-brand/10">
                    에이전트 상세 →
                  </Link>
                </div>
              )}
...
function Kv({ k, v, pos }: { k: string; v: string; pos?: boolean }) {
  const c = pos === undefined ? "text-ap-ink-1" : pos ? "text-ap-up" : "text-ap-down";
  return (
    <div className="bg-ap-bg border border-ap-line rounded px-2 py-1.5">
