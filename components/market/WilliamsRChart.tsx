...
function computeWilliamsR(bars: BarOut[], period: number): { time: UTCTimestamp; value: number }[] {
  ...
  for (let i = period - 1; i < bars.length; i++) {
    ...
    const wr = highestHigh === lowestLow ? -50 : ((highestHigh - bars[i].close) / (highestHigh - lowestLow)) * -100;
    result.push({ time: Math.floor(bars[i].ts_event / 1e9) as UTCTimestamp, value: wr });
  }
  return result;
}

export function WilliamsRChart({ bars, period = 14 }: WilliamsRChartProps) {
  ...
  return (
    <div className="border-t border-border bg-panel">
      <div className="px-4 py-1 flex items-center gap-2">
        <span className="text-[10px] text-text-3 uppercase tracking-wider">WILLIAMS %R ({period})</span>
      </div>
