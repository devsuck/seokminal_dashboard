interface LoadingStateProps {
  ...
  textClass?: string;
  /** Overrides Spinner's default border tone. */
  spinnerClass?: string;
}

/** 페이지/패널 공용 로딩 표시 — EmptyState와 동일 레이아웃, 스피너 포함. */
export function LoadingState({ message = "로딩 중…", hint, textClass = "text-text-3", spinnerClass }: LoadingStateProps) {
  return (
...
export function Spinner({ className }: { className?: string }) {
  return (
    <span
      className={`inline-block w-4 h-4 border-2 ${className ?? "border-border border-t-accent"} rounded-full animate-spin`}
      aria-label="로딩 중"/>
  );
}
