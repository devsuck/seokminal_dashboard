...
export function ChartFrame({ title, legend, caption, children, textClass = "text-text-3", legendTextClass = "text-text-2" }: {
  title?: string;
  legend?: LegendItem[];
  caption?: string;
  children: ReactNode;
  ...
  textClass?: string;
  /** Overrides default legend text-text-2 tone. */
  ...
  return (
    <div>
      ...
      {caption && <p className={`text-[9px] ${textClass} mt-1 font-data`}>{caption}</p>}
    </div>
  );
}
